...
public class QueryForwardsByGoalsStepDefinitions {
      ...
      @Given("there exists in the system players {string} with {int} goals and {string} with {int} goals")
      public void GivenPlayerExists(String Player1Name, int player1goals, String Player2Name, int player2goals) {
            ...
            player1.setGoals(player1goals); // Set a default value for goals
            player1.setAssists(2); // Set a default value for assists
            ...
            player2.setGoals(player2goals); // Set a default value for goals
            player2.setAssists(1); // Set a default value for assists
      ...
      public void whenISortPlayersByDescendingGoals() {
            ResponseEntity<List<ForwardResponseDto>> response = forwardController.getForwardsSortedByDescendingGoals();
            queriedPlayersDescending = response.getBody();
      }

      @Then("I am shown a descending goals list that has {string} ranked higher than {string}")
      ...
      public void whenISortPlayersByAscendingGoals() {
            ResponseEntity<List<ForwardResponseDto>> response = forwardController.getForwardsSortedByAscendingGoals();
            queriedPlayersAscending = response.getBody();
      }

      @Then("I am shown a ascending goals list that has {string} ranked higher than {string}")
      ...
      public void iQueryByAscendingOrderOfGoals() {
            responseEntity = forwardController.getForwardsSortedByAscendingGoals();
      }
