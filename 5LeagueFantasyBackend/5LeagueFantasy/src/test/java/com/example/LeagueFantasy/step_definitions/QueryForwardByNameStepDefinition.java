...
public class QueryForwardByNameStepDefinition {
    ...
    public void whenIQuery(String query){
        ...
        ResponseEntity<List<ForwardResponseDto>> responseEntity = forwardController.getForwardsByName(query);
    ...
    public void thenListIncludesPlayers(String expectedPlayer1, String expectedPlayer2) {
        Assert.assertTrue("Player list should include " + expectedPlayer1,
                queriedPlayers.stream().anyMatch(player -> player.getName().equals(expectedPlayer1)));

        Assert.assertTrue("Player list should include " + expectedPlayer2,
                queriedPlayers.stream().anyMatch(player -> player.getName().equals(expectedPlayer2)));
    }

    @Given("that the list of players in the system does not include any player whose name contains {string}")
